...
3. 进行消抖处理（首先判断读取的值于button的值是否不等于）
	1. 如果等于的话，就一直将cnt置0，不改变状态
	2. 如果不等于的话，表明电平状态在该tick`开始`发生变化
		1. 判断cnt+1是否大于`预设`防抖计数值（这个过程会一直将cnt+1）
			1. 如果小于，如果之后电平一直没变化，则tick一直加一，如果电平有变化，则什么也不做，当前tick不做任何改变，等到下个tick，如果读取的值依旧与button相等，则表示有抖动，将cnt=0，如果不同，则继续进行防抖
			2. 如果大于，则表示该轮防抖判断成功，将button的状态进行改变，cnt也置为0
4. 状态机（IDLE、PRESS、RELEASE、REPEAT、LONG_HOLD）