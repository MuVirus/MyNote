...
### 6、立即执行函数(IIFE)
#### 1. 为什么有 `(function() { ... })()` 这种写法？

这种写法是为了**立即执行一个函数**，同时**避免污染全局作用域**。
...
这个括号的作用是把函数变成**表达式**，而不是**声明语句**。在 Dart 中，函数声明和函数表达式是有区别的。
...
> 外面的两层 `()` 是为了把一个匿名函数变成表达式，并立即执行它，形成一个**私有作用域**，避免变量泄露到外部。

---
#### 类比一下：

你可以把它想象成一个“一次性”的函数盒子：
